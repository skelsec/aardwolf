...
fn process_plane(input: &mut dyn Read, width: u32, height: u32, output: &mut [u8]) -> Result<T, Error> {
    let mut indexw;
	let mut indexh= 0;
	let mut code ;
	let mut collen;
...
pub fn rle_32_decompress(input: &[u8], width: u32, height: u32, output: &mut [u8]) -> Result<T, Error> {
...
pub fn rle_16_decompress(input: &[u8], width: usize, mut height: usize, output: &mut [u16]) -> Result<T, Error> {
	let mut input_cursor = Cursor::new(input);
...
fn bitmap_decompress( input: &mut Vec<u8>, width: i16, height: i16, output: &mut Vec<u8>, bpp: usize) -> bool {
    let mut rv = false;
    match bpp {
        2 => rv = rle_16_decompress(input, width, height, output),
        4 => rv = rle_32_decompress(input, width, height, output),
        _ => (),
    }
    rv
}
...
fn bitmap_decompress_wrapper(py: Python<'_>, output: PyBuffer<u8>, width: usize, height: usize, input: PyBuffer<u8>, bitsperpixel: usize, bpp: usize, is_compressed: usize) -> PyResult<&PyByteArray> {
    ...
    if is_compressed != 0 {
        ...
        if !bitmap_decompress(&mut input.to_vec(py).unwrap(), width, height, &mut decomp_buffer, bpp) {
    ...
    if bitsperpixel == 16 {
        rgb565torgb32(decomp_buffer, width, height);
    }
