...
class RDPKerberos:
	...
	def setup(self):
		...
		if True: #self.settings.encrypt is 
			self.flags = \
	...
	async def setup_kc(self):
		try:
			...
			#if self.target.proxy is None or self.target.proxy.type in RDP_SOCKS_PROXY_TYPES:
			#	self.kc = AIOKerberosClient(self.ccred, self.target)

			#elif self.target.proxy.type in [RDPProxyType.MULTIPLEXOR, RDPProxyType.MULTIPLEXOR_SSL]:
			#	from aardwolf.network.multiplexor import MultiplexorProxyConnection
	...
	async def authenticate(self, authData, flags = None, seq_number = 0, cb_data = None, is_rpc = False):
		"""
