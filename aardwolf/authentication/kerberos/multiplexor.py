		...
		try:
			if is_rpc == True:
				if self.iterations == 0:
					...
					apreq, res = await self.ksspi.authenticate('termsrv/%s' % self.settings.target, flags = str(flags.value))

					self.iterations += 1
					return apreq, True, None
				
				elif self.iterations == 1:
					data, err = await self.ksspi.authenticate('termsrv/%s' % self.settings.target, flags = str(flags.value), token_data = authData)
					if err is not None:
				...
				apreq, res = await self.ksspi.authenticate('termsrv/%s' % self.settings.target)
