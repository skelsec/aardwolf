...
from aardwolf.authentication.ntlm.structures.ntlmssp_message_signature import NTLMSSP_MESSAGE_SIGNATURE
from aardwolf.authentication.ntlm.structures.ntlmssp_message_signature_noext import NTLMSSP_MESSAGE_SIGNATURE_NOEXT
			...
			if self.iteration_cnt == 0:
				...
				if self.settings.ntlm_downgrade == True:
					...
					if self.settings.credential.is_guest == True:
						...
						self.ntlmAuthenticate = NTLMAuthenticate.construct(self.flags, lm_response= lmresp, mic=None, encrypted_session = self.EncryptedRandomSessionKey)
					...
					if self.settings.credential.is_guest == True:
						...
						self.set_version(False)
						self.setup_crypto()
						self.ntlmAuthenticate = NTLMAuthenticate.construct(self.flags, lm_response= lmresp, encrypted_session = self.EncryptedRandomSessionKey)						
						return self.ntlmAuthenticate.to_bytes(), False, None
						
					else:
						#comment this out for testing!
						ti = self.ntlmChallenge.TargetInfo
						ti[AVPAIRType.MsvAvTargetName] = 'TERMSRV/%s' % ti[AVPAIRType.MsvAvDnsComputerName]
